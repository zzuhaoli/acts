...
static Ref_t create_detector(Detector &description, xml_h e,
  ...
  Volume endcapVol("endcap",
                   PolyhedraRegular(numsides, rmin, rmax, totalThickness), air);
  ...
  endcapVol.setAttributes(description, x_det.regionStr(), x_det.limitsStr(),
                          x_det.visStr());
  ...
  for (xml_coll_t xc(x_det, _U(layer)); xc; ++xc) {
    ...
    for (xml_coll_t xs(x_layer, _U(slice)); xs; ++xs) {
      ...
      Volume s_vol(s_name, PolyhedraRegular(numsides, rmin, rmax, s_thick),
                   s_mat);

      // s_vol.setVisAttributes(description.visAttributes(x_slice.visStr()));
      s_vol.setVisAttributes(description.visAttributes(x_det.visStr()));
      sliceZ += s_thick / 2;
    ...
    l_vol.setVisAttributes(description.visAttributes(x_det.visStr()));
    if (l_repeat <= 0)
