...
double rDefocus(double theta, double alpha, double rmin) {
  double r;
  if (theta == 2 * alpha)
    r = rmin / Cos(alpha);
  else
    r = rmin /
        Sqrt(1 + Cos(alpha) * Cos(alpha) * Tan(theta / 2) * Tan(theta / 2) -
             2 * Tan(theta / 2) * Sin(alpha) * Cos(alpha));
  return r;
}

double RDefocus(double theta, double alpha, double rmin) {
  double r = rDefocus(theta, alpha, rmin);
  double R = r * Cos(alpha);
  return R;
}

Trap *MyTrap(double theta0, double dtheta, double dphi, double b, double r,
             double R, double offsetw, double cthickness, bool iswrapping,
             bool isMiddle, bool isLeft) {
  double a, c, h1, h2, alpha1, alpha2, h3, h4, mtheta, mphi;
  if (isMiddle) {
    a = 5 - 2 * offsetw;
    c = a;
    h1 = (R - (b - 28) / 2.0) * Tan(dphi / 2) - offsetw;
    h2 = h1;
    alpha1 = 0;
    alpha2 = 0;
    h3 = (b + R - (b - 28) / 2.0) * Tan(dphi / 2) - offsetw;
    h4 = h3;
    mtheta = 0;
    mphi = 0;
    if (iswrapping) {
    ...
    a = (r - (b - 28) / 2.0 * Cos(theta0)) * Tan(dtheta) / Cos(theta0);
    c = a + b * Tan(dtheta);
    h1 = (R - (b - 28) / 2.0 * Cos(theta0)) * Tan(dphi / 2);
    h2 = ((R - (b - 28) / 2.0 * Cos(theta0)) + a * Sin(theta0)) * Tan(dphi / 2);
    alpha1 = 0;
    alpha2 = 0;
    h3 = (b * Cos(theta0) + (R - (b - 28) / 2.0 * Cos(theta0))) * Tan(dphi / 2);
    h4 = (c * Sin(theta0) + b * Cos(theta0) +
          (R - (b - 28) / 2.0 * Cos(theta0))) *
         Tan(dphi / 2);
    mtheta = ATan((c - a) / b / 2);
    mphi = M_PI / 2;
...
static Ref_t create_element(Detector &lcdd, xml_h e, SensitiveDetector sens) {
  ...
  Trapezoid stave(
      (RDefocus(dphi, dphi / 2, rmin - offsetr) - 0.5) * Tan(dphi / 2),
      (RDefocus(dphi, dphi / 2, rmin + offsetr) + 0.5 + sthickness) *
          Tan(dphi / 2),
      zmax, zmax, (sthickness + 1) / 2.0 + offsetr);
  ...
  rec::ECALBRecGeoData *GeoData = new rec::ECALBRecGeoData;
  ...
  for (int i = 0; i < 25; i++)
    GeoData->Theta.push_back(Theta0[i]);
  ...
  for (xml_coll_t clayer(x_det, _U(layer)); clayer; ++clayer) {
    ...
    for (int row = 1; row <= nRow; row++) {
      ...
      a = rDefocus(dphi, dphi / 2, rmin) * Tan(dtheta) / Cos(theta0);
      b = lthickness;
      c = a + b * Tan(dtheta);
      double y0 = (a + c) / 4;
      // double x0=0;
      ...
      y = y - rDefocus(dphi, dphi / 2, rmin) * Tan(theta0) - 2.5;
      z = z - sthickness / 2;
      double theta1;
      if (module_id > 26) {
        theta1 = theta0;
      ...
      if (module_id == 26)
        rowVol = Volume(
            rowStr,
            *MyTrap(theta0, dtheta, dphi, b + 1, rDefocus(dphi, dphi / 2, rmin),
                    RDefocus(dphi, dphi / 2, rmin), 0, 0, false, true, false),
            air);
      if (module_id > 26)
        rowVol = Volume(
            rowStr,
            *MyTrap(theta0, dtheta, dphi, b + 1, rDefocus(dphi, dphi / 2, rmin),
                    RDefocus(dphi, dphi / 2, rmin), 0, 0, false, false, false),
            air);
      if (module_id < 26)
        rowVol = Volume(
            rowStr,
            *MyTrap(theta0, dtheta, dphi, b + 1, rDefocus(dphi, dphi / 2, rmin),
                    RDefocus(dphi, dphi / 2, rmin), 0, 0, false, false, true),
            air);
          ...
          if (!x_module.isSensitive()) {
            ...
            if (x_module.id() == 1)
              moduleVol = new Volume(
                  moduleStr,
                  SubtractionSolid(*MyTrap(theta0, dtheta, dphi, b,
                                           rDefocus(dphi, dphi / 2, rmin),
                                           RDefocus(dphi, dphi / 2, rmin),
                                           awidth - mthickness, 0, false, true,
                                           false),
                                   *MyTrap(theta0, dtheta, dphi, b,
                                           rDefocus(dphi, dphi / 2, rmin),
                                           RDefocus(dphi, dphi / 2, rmin),
            ...
            if (x_module.id() == 2)
              moduleVol = new Volume(
                  ...
                  SubtractionSolid(
                      ...
                      *MyTrap(theta0, dtheta, dphi, b,
                              rDefocus(dphi, dphi / 2, rmin),
                              RDefocus(dphi, dphi / 2, rmin), awidth,
      ...
      rec::ECALBRecGeoData::RowPos rowpos;
      rowpos.x = x * 10;
      rowpos.y = y * 10;
      rowpos.z = z * 10;
      GeoData->Rows.push_back(rowpos);
  ...
  double offsetx = 0;
  double offsety1 = rDefocus(dphi, dphi / 2, rmin);
  double offsety2 = sthickness / 2;
  for (int i = 0; i < nLadders; i++) {
    double phi1 = dphi * i;
    double posx1 = offsetx * Cos(phi1) - offsety1 * Sin(phi1);
    double posx2 =
        offsetx * Cos(phi1 + dphi / 2) - offsety2 * Sin(phi1 + dphi / 2);
    double posy1 = offsetx * Sin(phi1) + offsety1 * Cos(phi1);
    double posy2 =
        offsetx * Sin(phi1 + dphi / 2) + offsety2 * Cos(phi1 + dphi / 2);

    rec::ECALBRecGeoData::StavePos stavepos;
    stavepos.x = (posx1 + posx2) * 10;
    stavepos.y = (posy1 + posy2) * 10;
    stavepos.z = 0;
    GeoData->Staves.push_back(stavepos);

    Transform3D tr(RotationZYX(0, -phi1 - dphi / 2, -M_PI / 2),
                   Position(posx1 + posx2, posy1 + posy2, 0));
  ...
  ECALBarrel.addExtension<rec::ECALBRecGeoData>(GeoData);

  return ECALBarrel;
}
DECLARE_DETELEMENT(STCFBEMC_03, create_element)
