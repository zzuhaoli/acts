...
static Ref_t create_element(Detector &description, xml_h e,
  ...
  std::string visStr = dim.visStr();
  std::string material = dim.attr<std::string>(_U(material));
  ...
  VolCylinder cylSurf(tube_vol, SurfaceType(SurfaceType::Helper),
                      0.5 * tube_thick, 0.5 * tube_thick, ocyl);
...
DECLARE_DETELEMENT(SCTube, create_element)
