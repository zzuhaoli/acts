...
void anaHisto(TH1F* inputHist, int j, TH1F* meanHist, TH1F* widthHist,
  ...
  if (inputHist->GetEntries() > 0) {
    if (fit) {
      ...
      widthHist->SetBinContent(j, inputHist->GetRMS());
...
void comparePerigee(std::string inFile1 =
  ...
  double yTitleOffset = plotResidual ? 1.9 : 1.5;
  ...
  //=======================================================================
  std::map<std::string, std::vector<TH1F*>> ref1_vs_eta;
  std::map<std::string, std::vector<TH1F*>> ref1_vs_pt;
  std::map<std::string, std::vector<TH1F*>> ref1_vs_d0;
  std::map<std::string, std::vector<TH1F*>> ref1_vs_z0;
  ...
  std::map<std::string, std::vector<TH1F*>> ref2_vs_eta;
  std::map<std::string, std::vector<TH1F*>> ref2_vs_pt;
  std::map<std::string, std::vector<TH1F*>> ref2_vs_d0;
  std::map<std::string, std::vector<TH1F*>> ref2_vs_z0;
  ...
  //=======================================================================
  std::map<std::string, std::vector<TH1F*>> val1_vs_eta;
  std::map<std::string, std::vector<TH1F*>> val1_vs_pt;
  std::map<std::string, std::vector<TH1F*>> val1_vs_d0;
  std::map<std::string, std::vector<TH1F*>> val1_vs_z0;
  ...
  std::map<std::string, std::vector<TH1F*>> val2_vs_eta;
  std::map<std::string, std::vector<TH1F*>> val2_vs_pt;
  std::map<std::string, std::vector<TH1F*>> val2_vs_d0;
  std::map<std::string, std::vector<TH1F*>> val2_vs_z0;
  ...
  std::vector<std::string> legs = {
      "EKF, W/O B field and material", "NLKF, W/O B field and material",
      "EKF, W/ B field and material", "NLKF, W/ B field and material",
      //"EKF, W/O B field and W/ material",
      //"NLKF, W/O B field and W/ material",
      //"EKF, W/ B field and material",
      //"NLKF, W/ B field and material",
  };
  ...
  if (1) {
    ...
    yRange_pullwidth_vs_eta = {
        {0.5, 3.}, {0.5, 4}, {0.5, 3}, {0.5, 4}, {0.8, 1.3}, {0.9, 1.2},
    };
  ...
  for (int i = 0; i < names.size(); ++i) {
    ...
    //  ref1_vs_pt.emplace(name, new TH2F(Form("ref1_%s_vs_pt", name.c_str()),
    //  "",
    //                                    pts.size() - 1, pts.front(),
    //                                    pts.back(), 100, yRange.first,
    //                                    yRange.second));
    //}
    // ref1_vs_d0.emplace(
    //                   name, new TH2F(Form("ref1_%s_vs_d0", name.c_str()), "",
    //                   100, -100, 100,
    //                                  100, yRange.first, yRange.second));
    // ref1_vs_z0.emplace(
    //                   name, new TH2F(Form("ref1_%s_vs_z0", name.c_str()), "",
    //                   100, -100, 100,
    //                                  100, yRange.first, yRange.second));

    for (int i = 0; i < etaBins; ++i) {
      ref1_vs_eta[name].push_back(
          new TH1F(Form("ref1_%s_in_etabin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }

    for (int i = 0; i < pts.size() - 1; ++i) {
      ref1_vs_pt[name].push_back(
          new TH1F(Form("ref1_%s_in_ptbin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }

    for (int i = 0; i < 100; ++i) {
      ref1_vs_d0[name].push_back(
          new TH1F(Form("ref1_%s_in_d0bin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }

    for (int i = 0; i < 100; ++i) {
      ref1_vs_z0[name].push_back(
          new TH1F(Form("ref1_%s_in_z0bin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }
    ...
    // ref2 plots
    ////////////////////////////
    //    ref2_vs_eta.emplace(name, new TH2F(Form("ref2_%s_vs_eta",
    //    name.c_str()), "",
    //                                       etaBins, -1.0 * etaRange, etaRange,
    //                                       100, yRange.first, yRange.second));
    //    if (variablePtBin) {
    //      ref2_vs_pt.emplace(name, new TH2F(Form("ref2_%s_vs_pt",
    //      name.c_str()), "",
    //                                        pts.size() - 1, pts.data(), 100,
    //                                        yRange.first, yRange.second));
    //    } else {
    ...
    //    }
    //    ref2_vs_d0.emplace(
    //                       name, new TH2F(Form("ref2_%s_vs_d0", name.c_str()),
    //                       "", 100, -100, 100,
    //                                      100, yRange.first, yRange.second));
    //    ref2_vs_z0.emplace(
    //                       name, new TH2F(Form("ref2_%s_vs_z0", name.c_str()),
    //                       "", 100, -100, 100,
    //                                      100, yRange.first, yRange.second));

    for (int i = 0; i < etaBins; ++i) {
      ref2_vs_eta[name].push_back(
          new TH1F(Form("ref2_%s_in_etabin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }

    for (int i = 0; i < pts.size() - 1; ++i) {
      ref2_vs_pt[name].push_back(
          new TH1F(Form("ref2_%s_in_ptbin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }

    for (int i = 0; i < 100; ++i) {
      ref2_vs_d0[name].push_back(
          new TH1F(Form("ref2_%s_in_d0bin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }

    for (int i = 0; i < 100; ++i) {
      ref2_vs_z0[name].push_back(
          new TH1F(Form("ref2_%s_in_z0bin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }
    ...
    // val1 plots
    ////////////////////////////
    //    val1_vs_eta.emplace(name, new TH2F(Form("val1_%s_vs_eta",
    //    name.c_str()), "",
    //                                       etaBins, -1.0 * etaRange, etaRange,
    //                                       100, yRange.first, yRange.second));
    //    if (variablePtBin) {
    //      val1_vs_pt.emplace(name, new TH2F(Form("val1_%s_vs_pt",
    //      name.c_str()), "",
    //                                        pts.size() - 1, pts.data(), 100,
    //                                        yRange.first, yRange.second));
    //    } else {
    ...
    //    }
    //    val1_vs_d0.emplace(
    //                       name, new TH2F(Form("val1_%s_vs_d0", name.c_str()),
    //                       "", 100, -100, 100,
    //                                      100, yRange.first, yRange.second));
    //    val1_vs_z0.emplace(
    //                       name, new TH2F(Form("val1_%s_vs_z0", name.c_str()),
    //                       "", 100, -100, 100,
    //                                      100, yRange.first, yRange.second));

    for (int i = 0; i < etaBins; ++i) {
      val1_vs_eta[name].push_back(
          new TH1F(Form("val1_%s_in_etabin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }

    for (int i = 0; i < pts.size() - 1; ++i) {
      val1_vs_pt[name].push_back(
          new TH1F(Form("val1_%s_in_ptbin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }

    for (int i = 0; i < 100; ++i) {
      val1_vs_d0[name].push_back(
          new TH1F(Form("val1_%s_in_d0bin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }

    for (int i = 0; i < 100; ++i) {
      val1_vs_z0[name].push_back(
          new TH1F(Form("val1_%s_in_z0bin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }
    ...
    // val2 plots
    ////////////////////////////
    //    val2_vs_eta.emplace(name, new TH2F(Form("val2_%s_vs_eta",
    //    name.c_str()), "",
    //                                       etaBins, -1.0 * etaRange, etaRange,
    //                                       100, yRange.first, yRange.second));
    //    if (variablePtBin) {
    //      val2_vs_pt.emplace(name, new TH2F(Form("val2_%s_vs_pt",
    //      name.c_str()), "",
    //                                        pts.size() - 1, pts.data(), 100,
    //                                        yRange.first, yRange.second));
    //    } else {
    ...
    //    }
    //    val2_vs_d0.emplace(
    //                       name, new TH2F(Form("val2_%s_vs_d0", name.c_str()),
    //                       "", 100, -100, 100,
    //                                      100, yRange.first, yRange.second));
    //    val2_vs_z0.emplace(
    //                       name, new TH2F(Form("val2_%s_vs_z0", name.c_str()),
    //                       "", 100, -100, 100,
    //                                      100, yRange.first, yRange.second));

    for (int i = 0; i < etaBins; ++i) {
      val2_vs_eta[name].push_back(
          new TH1F(Form("val2_%s_in_etabin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }

    for (int i = 0; i < pts.size() - 1; ++i) {
      val2_vs_pt[name].push_back(
          new TH1F(Form("val2_%s_in_ptbin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }

    for (int i = 0; i < 100; ++i) {
      val2_vs_d0[name].push_back(
          new TH1F(Form("val2_%s_in_d0bin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }

    for (int i = 0; i < 100; ++i) {
      val2_vs_z0[name].push_back(
          new TH1F(Form("val2_%s_in_z0bin_%d", name.c_str(), i + 1), "", 100,
                   yRange.first, yRange.second));
    }
  ...
  // StatOverflows(kTRUE) must be set before filling
  for (int i = 0; i < names.size(); ++i) {
    const auto& name = names[i];
    for (int j = 0; j < etaBins; ++j) {
      const auto& ref1 = ref1_vs_eta[name][j];
      const auto& ref2 = ref2_vs_eta[name][j];
      const auto& val1 = val1_vs_eta[name][j];
      const auto& val2 = val2_vs_eta[name][j];
      if (statOverflows) {
        ref1->StatOverflows(kTRUE);
        ref2->StatOverflows(kTRUE);
        val1->StatOverflows(kTRUE);
        val2->StatOverflows(kTRUE);
      }
    }
    for (int j = 0; j < pts.size() - 1; ++j) {
      const auto& ref1 = ref1_vs_pt[name][j];
      const auto& ref2 = ref2_vs_pt[name][j];
      const auto& val1 = val1_vs_pt[name][j];
      const auto& val2 = val2_vs_pt[name][j];
      if (statOverflows) {
        ref1->StatOverflows(kTRUE);
        ref2->StatOverflows(kTRUE);
        val1->StatOverflows(kTRUE);
  ...
  // Lambda to find the eta bin
  auto findEtaBin = [&](double eta) {
    double etaBinWidth = 2 * etaRange / etaBins;
    for (int i = 0; i < etaBins; ++i) {
      double etaLow = -1 * etaRange + etaBinWidth * i;
      double etaUp = -1 * etaRange + etaBinWidth * (i + 1);
      if (eta >= etaLow && eta < etaUp) {
        return i + 1;
      }
    }
    return -1;
  ...
  // Lambda to find the pt bin
  auto findPtBin = [&](double pt) {
    for (int i = 0; i < pts.size() - 1; ++i) {
      double ptLow, ptUp;
      if (variablePtBin) {
        ptLow = pts[i];
        ...
        double ptBinWidth = (pts.back() - pts.front()) / (pts.size() - 1);
        ptLow = pts.front() + ptBinWidth * i;
        ptUp = pts.front() + ptBinWidth * (i + 1);
      }
      if (pt >= ptLow && pt < ptUp) {
        return i + 1;
      }
  ...
  // Test findEtaBin
  std::cout << "-1 is in eta bin " << findEtaBin(-1) << std::endl;
  // Test findPtBin
  std::cout << "2.5 is in pt bin " << findPtBin(2.5) << std::endl;

  // Lambda to fill the plots
  auto fillHists = [&](unsigned long entries, const TrackSummaryReader& tracks,
                       std::map<std::string, std::vector<TH1F*>>& etaHists,
                       std::map<std::string, std::vector<TH1F*>>& ptHists) {
    for (unsigned long ie = 0; ie < entries; ++ie) {
      ...
      for (size_t it = 0; it < nTracks; ++it) {
        if (tracks.hasFittedParams->at(it)) {
          double truthPt = tracks.t_pT->at(it);
          double truthEta = tracks.t_eta->at(it);

          bool withinPt = truthPt >= ptMin and truthPt < ptMax;
          bool withinEta =
              absEta ? (std::abs(truthEta) >= absEtaMin and
                        std::abs(truthEta) < absEtaMax)
                     : (truthEta >= absEtaMin and truthEta < absEtaMax);

          int etaBin = findEtaBin(truthEta);
          int ptBin = findPtBin(truthPt);

          if (plotResidual) {
            if (withinPt and etaBin != -1) {
              etaHists["l0"][etaBin - 1]->Fill(tracks.res_eLOC0_fit->at(it));
              etaHists["l1"][etaBin - 1]->Fill(tracks.res_eLOC1_fit->at(it));
              etaHists["phi"][etaBin - 1]->Fill(tracks.res_ePHI_fit->at(it));
              etaHists["theta"][etaBin - 1]->Fill(
                  tracks.res_eTHETA_fit->at(it));
              etaHists["qop"][etaBin - 1]->Fill(tracks.res_eQOP_fit->at(it));
              etaHists["t"][etaBin - 1]->Fill(
                  timeInns(tracks.res_eT_fit->at(it)));
            }

            if (withinEta and ptBin != -1) {
              ptHists["l0"][ptBin - 1]->Fill(tracks.res_eLOC0_fit->at(it));
              ptHists["l1"][ptBin - 1]->Fill(tracks.res_eLOC1_fit->at(it));
              ptHists["phi"][ptBin - 1]->Fill(tracks.res_ePHI_fit->at(it));
              ptHists["theta"][ptBin - 1]->Fill(tracks.res_eTHETA_fit->at(it));
              ptHists["qop"][ptBin - 1]->Fill(tracks.res_eQOP_fit->at(it));
              ptHists["t"][ptBin - 1]->Fill(
                  timeInns(tracks.res_eT_fit->at(it)));
            }
          } else {
            if (withinPt and etaBin != -1) {
              etaHists["l0"][etaBin - 1]->Fill(tracks.pull_eLOC0_fit->at(it));
              etaHists["l1"][etaBin - 1]->Fill(tracks.pull_eLOC1_fit->at(it));
              etaHists["phi"][etaBin - 1]->Fill(tracks.pull_ePHI_fit->at(it));
              etaHists["theta"][etaBin - 1]->Fill(
                  tracks.pull_eTHETA_fit->at(it));
              etaHists["qop"][etaBin - 1]->Fill(tracks.pull_eQOP_fit->at(it));
              etaHists["t"][etaBin - 1]->Fill(tracks.pull_eT_fit->at(it));
            }

            if (withinEta and ptBin != -1) {
              ptHists["l0"][ptBin - 1]->Fill(tracks.pull_eLOC0_fit->at(it));
              ptHists["l1"][ptBin - 1]->Fill(tracks.pull_eLOC1_fit->at(it));
              ptHists["phi"][ptBin - 1]->Fill(tracks.pull_ePHI_fit->at(it));
              ptHists["theta"][ptBin - 1]->Fill(tracks.pull_eTHETA_fit->at(it));
              ptHists["qop"][ptBin - 1]->Fill(tracks.pull_eQOP_fit->at(it));
              ptHists["t"][ptBin - 1]->Fill(tracks.pull_eT_fit->at(it));
            }
          }
        }
      }
  ...
  for (int i = 0; i < names.size(); ++i) {
    ...
    for (int j = 1; j <= ref1mean_vs_eta["l0"]->GetNbinsX(); ++j) {
      ...
      //                                            Form("%s_projy_bin%d",
      //                                            "Pull_vs_eta_Histo", j), j,
      //                                            j);
      anaHisto(ref1_vs_eta[name][j - 1], j, ref1mean_vs_eta.at(name),
               ref1width_vs_eta.at(name));
    }
    for (int j = 1; j <= val1mean_vs_eta["l0"]->GetNbinsX(); ++j) {
      ...
      //                                            "Pull_vs_eta_Histo", j), j,
      //                                            j);
      anaHisto(val1_vs_eta[name][j - 1], j, val1mean_vs_eta.at(name),
               val1width_vs_eta.at(name));
    }
    for (int j = 1; j <= ref2mean_vs_eta["l0"]->GetNbinsX(); ++j) {
      ...
      //                                            "Pull_vs_eta_Histo", j), j,
      //                                            j);
      anaHisto(ref2_vs_eta[name][j - 1], j, ref2mean_vs_eta.at(name),
               ref2width_vs_eta.at(name));
    }
    for (int j = 1; j <= val2mean_vs_eta["l0"]->GetNbinsX(); ++j) {
      ...
      //                                            "Pull_vs_eta_Histo", j), j,
      //                                            j);
      anaHisto(val2_vs_eta[name][j - 1], j, val2mean_vs_eta.at(name),
               val2width_vs_eta.at(name));
    }
    ...
    for (int j = 1; j <= ref1mean_vs_pt["l0"]->GetNbinsX(); ++j) {
      ...
      //                                           Form("%s_projy_bin%d",
      //                                           "Pull_vs_pt_Histo", j), j,
      //                                           j);
      anaHisto(ref1_vs_pt[name][j - 1], j, ref1mean_vs_pt.at(name),
               ref1width_vs_pt.at(name));
    }
    for (int j = 1; j <= val1mean_vs_pt["l0"]->GetNbinsX(); ++j) {
      ...
      //                                           "Pull_vs_pt_Histo", j), j,
      //                                           j);
      anaHisto(val1_vs_pt[name][j - 1], j, val1mean_vs_pt.at(name),
               val1width_vs_pt.at(name));
    }
    for (int j = 1; j <= ref2mean_vs_pt["l0"]->GetNbinsX(); ++j) {
      ...
      //                                           "Pull_vs_pt_Histo", j), j,
      //                                           j);
      anaHisto(ref2_vs_pt[name][j - 1], j, ref2mean_vs_pt.at(name),
               ref2width_vs_pt.at(name));
    }
    for (int j = 1; j <= val2mean_vs_pt["l0"]->GetNbinsX(); ++j) {
      ...
      //                                           "Pull_vs_pt_Histo", j), j,
      //                                           j);
      anaHisto(val2_vs_pt[name][j - 1], j, val2mean_vs_pt.at(name),
               val2width_vs_pt.at(name));
    }
  }
  ...
  for (const auto& name : names) {
    ...
    if (plotType == 2) {
      legs_eta.push_back(new TLegend(0.3, 0.6, 0.9, 0.9));
      legs_pt.push_back(new TLegend(0.3, 0.6, 0.9, 0.9));
    } else {
      legs_eta.push_back(new TLegend(0.25, 0.7, 0.9, 0.9));
      legs_pt.push_back(new TLegend(0.25, 0.7, 0.9, 0.9));
    }
  ...
  auto plotHists = [&](std::map<std::string, TH1F*>& ref1Hists,
    ...
    for (int i = 0; i < names.size(); i++) {
      ...
      if (fit) {
        if (not plotMean) {
          ref1Hists[names[i]]->GetYaxis()->SetTitle(
              Form("Fitted #sigma of %s", leg.c_str()));
          val1Hists[names[i]]->GetYaxis()->SetTitle(
              Form("Fitted #sigma of %s", leg.c_str()));
        } else if (plotMean) {
          ref1Hists[names[i]]->GetYaxis()->SetTitle(
              Form("Fitted #mu of %s", leg.c_str()));
          val1Hists[names[i]]->GetYaxis()->SetTitle(
              Form("Fitted #mu of %s", leg.c_str()));
        }
      } else {
      ...
      if (labelSize == 0) {
        setThisHistStyle(ref1Hists[names[i]], 64, 24, xTitleSize, yTitleSize,
                         xLabelSize, yLabelSize, xTitleOffset, yTitleOffset);
        setThisHistStyle(val1Hists[names[i]], 95, 26, xTitleSize, yTitleSize,
                         xLabelSize, yLabelSize, xTitleOffset, yTitleOffset);
        setThisHistStyle(ref2Hists[names[i]], 64, 20, xTitleSize, yTitleSize,
                         xLabelSize, yLabelSize, xTitleOffset, yTitleOffset);
        setThisHistStyle(val2Hists[names[i]], 95, 22, xTitleSize, yTitleSize,
                         xLabelSize, yLabelSize, xTitleOffset, yTitleOffset);

      } else if (labelSize == 1) {
        setThisHistStyle(ref1Hists[names[i]], 64, 24, topXTitleSize,
                         topYTitleSize, topXLabelSize, topYLabelSize,
                         topXTitleOffset, topYTitleOffset, 505);
        setThisHistStyle(val1Hists[names[i]], 95, 26, topXTitleSize,
                         topYTitleSize, topXLabelSize, topYLabelSize,
                         topXTitleOffset, topYTitleOffset, 505);
        setThisHistStyle(ref2Hists[names[i]], 64, 20, topXTitleSize,
                         topYTitleSize, topXLabelSize, topYLabelSize,
                         topXTitleOffset, topYTitleOffset, 505);
        setThisHistStyle(val2Hists[names[i]], 95, 22, topXTitleSize,
                         topYTitleSize, topXLabelSize, topYLabelSize,
                         topXTitleOffset, topYTitleOffset, 505);

      } else if (labelSize == 2) {
        setThisHistStyle(ref1Hists[names[i]], 64, 24, botXTitleSize,
                         botYTitleSize, botXLabelSize, botYLabelSize,
                         botXTitleOffset, botYTitleOffset, 505);
        setThisHistStyle(val1Hists[names[i]], 95, 26, botXTitleSize,
                         botYTitleSize, botXLabelSize, botYLabelSize,
                         botXTitleOffset, botYTitleOffset, 505);
        setThisHistStyle(ref2Hists[names[i]], 64, 20, botXTitleSize,
                         botYTitleSize, botXLabelSize, botYLabelSize,
                         botXTitleOffset, botYTitleOffset, 505);
        setThisHistStyle(val2Hists[names[i]], 95, 22, botXTitleSize,
                         botYTitleSize, botXLabelSize, botYLabelSize,
                         botXTitleOffset, botYTitleOffset, 505);
      }
      ...
      if (plotResidual and not plotMean and plotResidualRatio) {
        ...
        // ref1Hists[names[i]]->Draw("E4");
        // val1Hists[names[i]]->Draw("E4 same");
        // ref2Hists[names[i]]->Draw("E4 same");
        // val2Hists[names[i]]->Draw("E4 same");
        // ref1Hists[names[i]]->SetFillColor(64);
        // val1Hists[names[i]]->SetFillColor(95);
        // ref1Hists[names[i]]->SetFillStyle(3354);
        // val1Hists[names[i]]->SetFillStyle(3154);
        // ref1Hists[names[i]]->SetFillColorAlpha(64, 0.571);
        // val1Hists[names[i]]->SetFillColorAlpha(95, 0.571);
      }
        ...
        if (plotEta) {
          if (plotResidualRatio) {
            legs_eta[i]->AddEntry(val1Hists[names[i]],
                                  "W/O B field and material", "lep");
            legs_eta[i]->AddEntry(val2Hists[names[i]], "W B field and material",
                                  "lep");
          } else {
            legs_eta[i]->AddEntry(ref1Hists[names[i]], legs[0].c_str(), "lep");
            legs_eta[i]->AddEntry(ref2Hists[names[i]], legs[2].c_str(), "lep");
            legs_eta[i]->AddEntry(val1Hists[names[i]], legs[1].c_str(), "lep");
            legs_eta[i]->AddEntry(val2Hists[names[i]], legs[3].c_str(), "lep");
          }
          legs_eta[i]->Draw();
          // myText(0.6, 0.5, 1, 0.06, Form("%.1f GeV < p_{T} < %.1f GeV",
          // ptMin, ptMax));
          ...
          if (plotResidualRatio) {
            legs_pt[i]->AddEntry(val1Hists[names[i]],
                                 "W/O B field and material", "lep");
            legs_pt[i]->AddEntry(val2Hists[names[i]], "W B field and material",
                                 "lep");
          } else {
          ...
          legs_pt[i]->Draw();
          // myText(0.6, 0.5, 1, 0.06, Form("%.1f < |#eta| < %.1f", absEtaMin,
          // absEtaMax));
        }
      }
