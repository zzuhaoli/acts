...
ProcessCode TruthTrackFinder::execute(const AlgorithmContext& ctx) const {
  ...
  for (const auto& particle : particles) {
    ...
    if (m_cfg.removeHitsFromLoops) {
      ...
      std::cout << "There are " << trackHitList.size()
                << " sim hits for the particle " << std::endl;
      ...
      std::cout << "After removing hits from loops, there are "
                << trackHitFilteredList.size() << " sim hits for the particle "
                << std::endl;
