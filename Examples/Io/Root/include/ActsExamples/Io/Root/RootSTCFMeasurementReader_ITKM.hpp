...
class RootSTCFMeasurementReader_ITKM : public IReader {
 public:
  ...
  RootSTCFMeasurementReader_ITKM(const Config& config, Acts::Logging::Level level);

  /// Destructor
  ~RootSTCFMeasurementReader_ITKM();
 ...
 private:
  ...
  std::array<Acts::ActsScalar, 3> m_ITKRadius = {37.5004, 99.0104,160.759};
  std::vector<int> m_nLadders = {12, 32, 52};
  std::vector<int> m_nSectors = {12, 30, 48};
  ...
  TTreeReaderArray<int>* ITKladderID = nullptr;
  TTreeReaderArray<int>* ITKsectorID = nullptr;
