...
ActsExamples::RootSTCFMeasurementReader_BKG::RootSTCFMeasurementReader_BKG(
  ...
  if (m_cfg.outputParticles.empty()) {
    throw std::invalid_argument("Missing simulated particles output collection");
  }
...
ActsExamples::ProcessCode ActsExamples::RootSTCFMeasurementReader_BKG::read(
  ...
  if (m_particle_treeReader != nullptr && m_simhit_treeReader != nullptr&& context.eventNumber < m_events) {
	...
	Acts::Vector3 mom((*MDCmomentumX)[i], (*MDCmomentumY)[i], (*MDCmomentumZ)[i]); 
      ...
      for (auto simHitsGroup : groupByModule(simHits)) {
	    ...
	    //Acts::ActsVector<2> par{ driftDistance + sigma* stdNormal(rng), lPosition[1] + 5*stdNormal(rng)};
          ...
          measurementParticlesMap.emplace_hint(
                      measurementParticlesMap.end(), measurementIdx, particleId);
	...
	Acts::Vector3 pos((*particleVertexX)[i], (*particleVertexY)[i], (*particleVertexZ)[i]);
        ...
        double charge = 0; 
	if((*particlePDG)[i]==13 or (*particlePDG)[i]==11 or (*particlePDG)[i]==-211 or (*particlePDG)[i]== -2212 ){
           charge = -1;	
	}else if ((*particlePDG)[i]==-13 or (*particlePDG)[i]==-11 or (*particlePDG)[i]==211 or (*particlePDG)[i]== 2212){
           charge = 1;	
	}	
	ActsFatras::Particle particle(ActsFatras::Barcode(i),
	...
	particle.setITKHits(particleITKnum);//////******set  particle ITKHits
        ...
        particle.setPosition4(
            ...
            pos.z() * Acts::UnitConstants::mm, 0 * Acts::UnitConstants::ns);
        //// Only used for direction; normalization/units do not matter
        particle.setDirection(mom.x(), mom.y(), mom.z()); // in GeV
        particle.setAbsoluteMomentum(std::sqrt(mom.x()*mom.x() + mom.y()*mom.y() + mom.z()*mom.z()) *
