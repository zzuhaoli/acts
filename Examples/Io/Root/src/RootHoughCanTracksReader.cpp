	  ...
	  m_treeReader(nullptr)
{
    ...
    initialPositionX = new TTreeReaderArray<Acts::ActsScalar>(*m_treeReader,"CanTrackCol.initialPos.x");
    initialPositionY = new TTreeReaderArray<Acts::ActsScalar>(*m_treeReader,"CanTrackCol.initialPos.y");
    initialPositionZ = new TTreeReaderArray<Acts::ActsScalar>(*m_treeReader,"CanTrackCol.initialPos.z");
    initialMomentumX = new TTreeReaderArray<Acts::ActsScalar>(*m_treeReader,"CanTrackCol.initialMom.x");
    initialMomentumY = new TTreeReaderArray<Acts::ActsScalar>(*m_treeReader,"CanTrackCol.initialMom.y");
    initialMomentumZ = new TTreeReaderArray<Acts::ActsScalar>(*m_treeReader,"CanTrackCol.initialMom.z");
    chiSquare = new TTreeReaderArray<double>(*m_treeReader,"CanTrackCol.chiSquare");
...
ActsExamples::ProcessCode ActsExamples::RootHoughCanTracksReader::read(const ActsExamples::AlgorithmContext& context)
{
  ...
  if(m_treeReader !=nullptr && context.eventNumber < m_events)
  {
   ...
   if(m_treeReader->Next())
   {
    ...
    {
       Acts::Vector3 inipos(
	      (*initialPositionX)[i]*Acts::UnitConstants::mm,
       ...
       const auto [newPhi, newTheta] = Acts::detail::normalizePhiTheta(phi + sigmaPhi * stdNormal(rng), theta + sigmaTheta * stdNormal(rng));
